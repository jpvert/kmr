...
#'   sequence of values or "\code{lambda.min}". Default is "\code{lambda.min}".
#'   Note that "\code{lambda.min}" will allow different lambdas tuned for each 
#'   task while specified value(s) of lambda will be fixed for all tasks.
#'   
...
#'   values of \code{lambda} in the list. If \code{lambda} is "\code{lambda.min}",
#'   a single matrix returned too.
#' @export
predict.cv.kmr <- function(object, newx, lambda = "lambda.min") {
  ...
  # or just the prediction matrix if there is a single lambda or "lambda.min"
  if (length(pred)==1) {
    return(pred[[1]])
